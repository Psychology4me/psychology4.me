...
${$$result.head += `<!-- HEAD_svelte-13jnlyq_START -->${$$result.title = `<title>Blog Category: ${escape(category)}</title>`, ""}<!-- HEAD_svelte-13jnlyq_END -->`, ""}
 

${posts.length ? `<section class="${"padding-top-10"}"><div class="${"content"}"><div class="${"container-l"}"><div class="${"container-m"}"><h1 class="${"heading-5 text-center padding-top-10"}">Category: ${escape(category)}</h1>
    ...
    {},
    {}
  )}</div></div></section>` : `<section class="${"slide padding-bottom-10"}"><div class="${"content"}"><div class="${"container-l text-center"}"><h1 class="${"heading-5"}">Oops!</h1>

		 <p>Sorry, couldn&#39;t find any posts in the category &quot;${escape(category)}&quot;. <br> <a href="${"/blog"}" class="${"link-underlined"}">Bring me back</a></p></div></div></section>`}`;
});
export {
  Page as default
};
